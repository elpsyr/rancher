...
GOOS="${TARGET_OS}" GOARCH="${TARGET_ARCH}" CGO_ENABLED=0 "${GO_BINARY}" build -tags k8s \
    -ldflags "-X github.com/rancher/rancher/pkg/version.Version=2.8.5 -X github.com/rancher/rancher/pkg/version.GitCommit=2.8.5 -X github.com/rancher/rancher/pkg/settings.InjectDefaults=$DEFAULT_VALUES -extldflags -static -s" \
    -o "${RANCHER_BINARY}"
...
export http_proxy="http://192.168.3.90:7890"
export HTTP_PROXY="http://192.168.3.90:7890"
export https_proxy="http://192.168.3.90:7890"
export HTTPS_PROXY="http://192.168.3.90:7890"

sed -i 's/SAMEORIGIN/ALLOWALL/g' vendor/github.com/rancher/apiserver/pkg/middleware/cache.go
...
DOCKERFILE="${SCRIPT_DIR}/../package/Dockerfile_cfel"
...
docker buildx build -t "${TARGET_REPO}" -f "${DOCKERFILE}" \
  ...
  "${PACKAGE_FOLDER}" --platform="${TARGET_OS}/${TARGET_ARCH}" --network host
